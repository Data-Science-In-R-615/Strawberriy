...
```{r}
strawberry$Domain <- sub(",.*", "", strawberry$Domain)


strawberry <- strawberry |> 
  separate(col = `Fruit`, 
           into = c(NA, "Fruit"),  # NA for the part you want to discard, keep the rest
           sep = "-", 
           extra = "merge",  # In case there are multiple hyphens, merge everything after the first hyphen
           fill = "right")  # Handle cases where no hyphen exists
...
```{r}
strawberry <- strawberry |> 
  separate(col = `Domain Category`, 
           into = c(NA, "Domain Category"),  # NA for the part you want to discard, keep the rest
           sep = ",", 
           extra = "merge",  # In case there are multiple hyphens, merge everything after the first hyphen
           fill = "right")  # Handle cases where no hyphen exists
